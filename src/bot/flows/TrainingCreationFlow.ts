...
export class TrainingCreationFlow {
  ...
  private async handleQuickTrainingCreation(
    ...
    // Минимум 5 параметров: дата / мин / макс / стоимость / заметки
    // Локация опциональна - если не указана, запросим на следующем шаге
    if (parts.length < 5) {
      await ctx.reply(
        '❌ Недостаточно параметров для быстрого создания.\n\n' +
          'Используйте формат (локация опциональна):\n' +
          'дата / мин / макс / стоимость / заметки / локация\n\n' +
          'Пример: 10.02 18:00 / 5 / - / - / - / Зал\n' +
          'Или: 10.02 18:00 / 5 / - / - / приходите пораньше'
      );
      return;
    }
    ...
    // Локация (опционально в быстром формате)
    if (parts.length > 5 && parts[5] && parts[5] !== '-') {
      ...
      // Локация не указана - запрашиваем на следующем шаге
      state.step = 'location';
